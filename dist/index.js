...
Error Handling:
  ...
  - Returns error if syntax is invalid`,
    ...
    try {
        ...
        const dataLines = result.rows.map((row) => `| ${headers.map((h) => {
            const val = row[h];
            if (val === null)
                return "NULL";
...
Returns:
  Table listing with name, estimated row count, and disk size.`,
    ...
    try {
        ...
        ORDER BY COALESCE(s.n_live_tup, 0) DESC`, [schema]);
        if (result.rows.length === 0) {
            return {
        ...
        for (const row of result.rows) {
            lines.push(`| ${row.table_name} | ${Number(row.estimated_rows).toLocaleString()} | ${row.total_size} | ${row.comment || "—"} |`);
        }
...
Returns:
  Detailed table structure with column definitions, primary keys, foreign keys, and indexes.`,
    ...
    try {
        // Columns
        const columns = await executeQuery(`SELECT
          c.column_name,
          c.data_type,
        ...
        // Primary key
        const pk = await executeQuery(`SELECT kcu.column_name
        FROM information_schema.table_constraints tc
        JOIN information_schema.key_column_usage kcu
          ON tc.constraint_name = kcu.constraint_name
        WHERE tc.table_schema = $2 AND tc.table_name = $1 AND tc.constraint_type = 'PRIMARY KEY'
        ORDER BY kcu.ordinal_position`, [table, schema]);
        // Foreign keys
        const fks = await executeQuery(`SELECT
          kcu.column_name,
        ...
        WHERE tc.table_schema = $2 AND tc.table_name = $1 AND tc.constraint_type = 'FOREIGN KEY'`, [table, schema]);
        // Indexes
        const indexes = await executeQuery(`SELECT indexname, indexdef
        FROM pg_indexes
        WHERE schemaname = $2 AND tablename = $1`, [table, schema]);
        // Format output
        const pkCols = new Set(pk.rows.map((r) => r.column_name));
        const fkMap = new Map(fks.rows.map((r) => [
            r.column_name,
        ...
        for (const col of columns.rows) {
            ...
            lines.push(`| ${col.column_name} | ${typeName} | ${col.is_nullable} | ${col.column_default || "—"} | ${pkCols.has(col.column_name) ? "✅" : ""} | ${fkMap.get(col.column_name) || ""} | ${col.comment || ""} |`);
        }
...
// ============================================================================
server.registerTool("pg_explain_query", {
...
// ============================================================================
server.registerTool("pg_table_stats", {
...
Returns:
  Table health statistics useful for performance analysis.`,
    ...
    try {
        ...
        WHERE schemaname = $2 AND relname = $1`, [table, schema]);
        if (result.rows.length === 0) {
            return {
...
// ============================================================================
server.registerTool("pg_list_indexes", {
...
Returns:
  Index listing with definitions, sizes, and scan counts.`,
    ...
    try {
        ...
        ORDER BY COALESCE(s.idx_scan, 0) DESC`, [table, schema]);
        if (result.rows.length === 0) {
            return {
